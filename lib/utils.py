...
try:
    ...
    print("Please install dlib first.")
    quit()
...
def get_picture_locations(picture_numpy_array, loop_times=1, handler_model="hog"):
    ...
    Args:
      ...
      handler_model: Which face detection model to use. ('hog' or 'cnn', 'hog' is less accurate but faster on CPUs.)
...
def _raw_face_locations_batched(images, number_of_times_to_upsample=1):
    ...
    :return: A list of dlib 'rect' objects of found face locations
    """
    return cnn_detector(images, number_of_times_to_upsample)
